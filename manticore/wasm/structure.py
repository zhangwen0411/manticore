...
class WASIEnvironmentConfig:
    """WASI environment configuration, passed to the constructor of WASIEnvironment.  Immutable."""
    args: tuple[str, ...] = None  # Command-line arguments.
...
class WASIEnvironment:
    ...
    @staticmethod
    def _to_bytes(bss: typing.List[typing.Union[int, bytes]]) -> bytes:
        lb: typing.List[bytes] = []
        for bs in bss:
    ...
    def __init__(self, filename: str, wasi_config: WASIEnvironmentConfig = None) -> None:
        if wasi_config is None:
            wasi_config = WASIEnvironmentConfig()

        if (args := wasi_config.args) is None:
            args = (filename,)
        self.argv = tuple(arg.encode("ascii") + b'\0' for arg in args)

        self.exit_result = None
        self.stdin = wasi_config.stdin or b''
        self.stdout: typing.List[bytes] = []

        functions = {func_name: partial(self.func_missing, func_name) for func_name in self.ALL_FUNCTION_NAMES}
    ...
    def _consume_stdin(self, state, nbytes: int):
        if isinstance(self.stdin, typing.Callable):
            self.stdin = self.stdin(state)
        ret = self.stdin[:nbytes]
        self.stdin = self.stdin[nbytes:]
        return ret
    ...
    def _wasi_args_sizes_get(self, state, argc_ptr, argv_size_ptr):
        state.mem.write_int(argc_ptr, len(self.argv))
        state.mem.write_int(argv_size_ptr, sum(len(s) for s in self.argv))
        return [0]

    def _wasi_args_get(self, state, argv_ptr, argv_buf_ptr):
        for argb in self.argv:
            state.mem.write_int(argv_ptr, argv_buf_ptr)
            argv_ptr += 4

            state.mem.write_bytes(argv_buf_ptr, argb)
            argv_buf_ptr += len(argb)
    ...
    def _wasi_fd_read(self, state, fd, iovs, iovs_len, out_ptr):
        ...
        bs = self._consume_stdin(state, buf_len)
        state.mem.write_bytes(buf_ptr, bs)
        state.mem.write_int(out_ptr, len(bs))
        return [0]
    ...
    def _wasi_fd_write(self, state, fd, iovs, iovs_len, out_ptr):
        ...
        for i in range(iovs_len):
            ...
            buf_bytes = self._to_bytes(state.mem.read_bytes(buf_ptr, buf_len))
